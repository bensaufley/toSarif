...
type Sarif22SchemaJson struct {
	...
	Version Sarif22SchemaJsonVersion `json:"version" yaml:"version" mapstructure:"version"`
}

type Sarif22SchemaJsonVersion string

const Sarif22SchemaJsonVersionA22 Sarif22SchemaJsonVersion = "2.2"

var enumValues_Sarif22SchemaJsonVersion = []interface{}{
	"2.2",
}

// UnmarshalJSON implements json.Unmarshaler.
func (j *Sarif22SchemaJsonVersion) UnmarshalJSON(b []byte) error {
	var v string
	if err := json.Unmarshal(b, &v); err != nil {
		return err
	}
	var ok bool
	for _, expected := range enumValues_Sarif22SchemaJsonVersion {
		if reflect.DeepEqual(v, expected) {
			ok = true
			break
		}
	...
	if !ok {
		return fmt.Errorf("invalid value (expected one of %#v): %#v", enumValues_Sarif22SchemaJsonVersion, v)
	}
	*j = Sarif22SchemaJsonVersion(v)
	return nil
}

// UnmarshalJSON implements json.Unmarshaler.
func (j *Sarif22SchemaJson) UnmarshalJSON(b []byte) error {
	var raw map[string]interface{}
	if err := json.Unmarshal(b, &raw); err != nil {
		return err
	}
	if _, ok := raw["runs"]; raw != nil && !ok {
		return fmt.Errorf("field runs in Sarif22SchemaJson: required")
	}
	if _, ok := raw["version"]; raw != nil && !ok {
		return fmt.Errorf("field version in Sarif22SchemaJson: required")
	}
	type Plain Sarif22SchemaJson
	var plain Plain
	if err := json.Unmarshal(b, &plain); err != nil {
		return err
	}
	*j = Sarif22SchemaJson(plain)
	return nil
}
